...
# from pyspark.sql.functions import col, from_json, to_timestamp, concat_ws, when
...
# # ─────────────────────────────────────────────
# # 2. Schema — mirrors your actual Kafka payload
# # ─────────────────────────────────────────────
...
#     .add("source_icon",      StringType()) \
#     .add("sentiment",        StringType()) \
#     .add("sentiment_stats",  StringType()) \
#     .add("ai_tag",           StringType()) \
#     .add("ai_region",        StringType()) \
#     .add("ai_org",           StringType()) \
#     .add("ai_summary",       StringType()) \
...
#     .withColumn(
#         "pubDate",
#         to_timestamp(col("pubDate"), "yyyy-MM-dd HH:mm:ss")
#     ) \
#     .withColumn(
#         "fetched_at",
#         to_timestamp(col("fetched_at"), "yyyy-MM-dd HH:mm:ss")
#     ) \
...
#     .withColumn(
#         "content",                                  # mask paywalled fields
#         when(col("content") == "ONLY AVAILABLE IN PAID PLANS", None)
#         .otherwise(col("content"))
#     ) \
#     .withColumn(
#         "ai_summary",
#         when(col("ai_summary").startswith("ONLY AVAILABLE"), None)
#         .otherwise(col("ai_summary"))
#     ) \
#     .withColumn(
#         "sentiment",
#         when(col("sentiment").startswith("ONLY AVAILABLE"), None)
#         .otherwise(col("sentiment"))
#     ) \
#     .dropDuplicates(["article_id"])                 # deduplicate on article_id

# # ─────────────────────────────────────────────
# # 6. Select only the columns you care about
...
# # ─────────────────────────────────────────────
# # 7. Output — swap format for your sink
# #    e.g. "parquet", "delta", "jdbc", etc.
# # ─────────────────────────────────────────────
...
#     col, from_json, to_timestamp, concat_ws,
#     when, trim, lit, length
# )
# from pyspark.sql.types import (
#     StructType, StringType, ArrayType, BooleanType, IntegerType
# )
...
# # ─────────────────────────────────────────────
# # 6. Final column selection — 18 clean fields
# # ─────────────────────────────────────────────
...
# # 7. Write stream to PostgreSQL
# # ─────────────────────────────────────────────

# def write_to_postgres(batch_df, batch_id):
#     batch_df.write \
#         .format("jdbc") \
#         .option("url", "jdbc:postgresql://localhost:5432/newsdb") \
